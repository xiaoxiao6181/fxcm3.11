            ...
            $.each(res_list, function (k, v) {
                ...
                html += '<li ng-repeat="o" style="display: flex;" onclick="get_hold_order(' + v.id + ')">\
                    <section style="flex:1;width:auto;">\
                        <p style="margin: 0;display: flex;">
                            <span class="ng-binding" style="width:auto;margin-right: 5px;">' + v.ptitle + '</span>
                            <span style="align-items: center;display: flex;" class="ng-binding ' + ostyle_class2 + '">
...
function get_hold_order(oid) {
    ...
    $.get(url, function (data) {
        ...
        $('.order-modal-content .fangxiang').html(
            data.ostyle ?
                (I18n.t('product.buyDown') + '/') + data.endprofit + '秒' :
                (I18n.t('product.buyUp') + '/') + data.endprofit + '秒'
        );
        ...
        $('.modal-backdrop').removeClass('ng-hide').addClass('active');
        $('.tab-nav').hide();
    })
}

function close_order_modal() {
    $('.modal-backdrop').addClass('ng-hide').removeClass('active');
    $('.tab-nav').show();
}
